...
$characters = $sql->mapear_Episodes_Characters();
...
function insert($characters, $episodes, $locations) {
    ...
    for ($k = 0; $k < count($locations); $k++) {
        $sql->insertLocations($locations[$k]["id"], $locations[$k]["name"], $locations[$k]["type"], $locations[$k]["dimension"], $locations[$k]["created"]);
    }
}
